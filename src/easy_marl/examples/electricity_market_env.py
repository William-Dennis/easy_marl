...
from easy_marl.examples.oberservators import OBSERVERS
...
class ElectricityMarketEnv(BaseMarketEnv):
    ...
    def _initialize_market_parameters(self, params: Dict[str, Any]) -> None:
        ...
        self.demand_scale = np.max(self.D_profile) if np.max(self.D_profile) > 0 else 1.0
    ...
    def _render_implementation(self) -> None:
        ...
        print(f"  Agent {agent_idx} quantity: {self.output['q_cleared'][t_idx, agent_idx]:.2f}")
        print(f"  Market price: {self.output['market_prices'][t_idx]:.2f}")
        print(f"  Agent {agent_idx} reward: {self.output['rewards'][t_idx, agent_idx]:.4f}")
